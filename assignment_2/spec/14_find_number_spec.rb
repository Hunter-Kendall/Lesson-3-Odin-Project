...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    # explained above.
    let(:number_guessing) { double(value: 8) }
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_5) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        expect(game_5.make_guess).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_5) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        expect(game_5.make_guess).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game_5) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        expect(game_5.make_guess).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_5) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        expect(game_5.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are equal' do
      let(:end_number) { double(value: 3) }
      subject(:game_guessing) { described_class.new(0, 9, end_number, 3) }
    ...
    context 'when guess and random_number are not equal' do
      let(:wrong_number) { double(value: 4) }
      subject(:game_guessing) { described_class.new(0, 9, wrong_number, 3) }
