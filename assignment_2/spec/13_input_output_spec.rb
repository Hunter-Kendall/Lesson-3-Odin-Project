...
# Long methods are like a run-on sentence that should have been divided into 2 or 3 different sentences so that everything could 
# be clearly understood and in this case if a method does many different things it can be difficult to test.
...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    subject(:game_not_end) { described_class.new(3, '5') }
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      # remove the 'x' before running this test
      subject(:game_2) { described_class.new(5, '5', 3) }
      it 'outputs correct phrase' do
        congrats_phrase = "Congratulations! You picked the random number in 3 guesses!\n"
        expect { game_2.final_message }.to output(congrats_phrase).to_stdout
      end
    end
    ...
    context 'when count is 4 and over' do
      # remove the 'x' before running this test
      subject(:game_3) { described_class.new(5, '5', 4) }
      it 'outputs correct phrase' do
        congrats_phrase = "That was hard. It took you 4 guesses!\n"
        expect { game_3.final_message }.to output(congrats_phrase).to_stdout
      end
    end
  end
end
